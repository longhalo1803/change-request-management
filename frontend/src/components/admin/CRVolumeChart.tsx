...
export const CRVolumeChart = ({ data, onAnnualProjection, growthPercentage = 14 }: CRVolumeChartProps) => {
  return (
    <div className="bg-white rounded-lg p-6 border border-gray-100 shadow-sm">
      {/* Header */}
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-lg font-semibold text-gray-800">CR Volume Trends by Priority</h3>
      ...
      <div className="mt-6 flex justify-between px-4 text-xs text-gray-500">
        {data.map((item) => (
          <div key={item.month} className="text-center text-xs font-semibold text-gray-600">
            {item.total}
          </div>
        ))}
      </div>
