...
const getStatusConfig = (status: CrStatus) => {
  const configMap: Record<CrStatus, { color: string; text: string }> = {
    ...
    [CrStatus.UNDER_ANALYSIS]: { color: 'orange', text: 'In Discussion' },
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_: any, record: ChangeRequest) => {
        ...
        } else if (record.status === CrStatus.UNDER_ANALYSIS) {
          return (
            <div className="flex gap-2">
              <Button 
                danger
      ...
      {pagination && (
        <div className="flex items-center justify-between px-6 py-4 border-t">
          <div className="text-sm text-gray-600">
            Showing {((pagination.current - 1) * pagination.pageSize) + 1} - {Math.min(pagination.current * pagination.pageSize, pagination.total)} of {pagination.total} entries
          </div>
          <Pagination
